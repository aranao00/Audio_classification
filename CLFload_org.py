...
try:
    ...
    from config_load_trndec import config
    import numpy as np
    import random
    # Seed setting
    def set_seed(num=42):
    ...
    class Attention_clf(nn.Module):
        def __init__(self, hdim, layer_num, token_number, nhead=32, febase=config['FEmodel']):
            ...
            self.dl=nn.TransformerDecoderLayer(d_model=self.hdim, nhead=nhead)
            if hdim!=self.dim:
            ...
            for i in range(layer_num):
                self.layer.append(nn.TransformerDecoderLayer(d_model=self.hdim, nhead=nhead))
                # if i-1==layer_num:
                #     break
